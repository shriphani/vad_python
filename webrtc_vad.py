...
if version_info >= (2,6,0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_webrtc_vad', [dirname(__file__)])
        except ImportError:
            import _webrtc_vad
            return _webrtc_vad
        if fp is not None:
            try:
                _mod = imp.load_module('_webrtc_vad', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _webrtc_vad = swig_import_helper()
